...
hdhome_cookie = os.environ["HDHOME_COOKIE"]

if not hdhome_cookie:
...
for indexer in indexers:
    if indexer.get("definitionName") != "hdhome":
        continue
    ...
    for i in range(len(indexer["fields"])):
        ...
        if indexer["fields"][i]["value"] == hdhome_cookie:
            print("Cookie is up to date")
            exit(0)
        indexer["fields"][i]["value"] = hdhome_cookie
