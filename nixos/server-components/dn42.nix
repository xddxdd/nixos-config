{
  ...
  setupAddressing = interfaceName: v: let
    mtu =
...
in {
  ...
  config.networking.wireguard.interfaces = let
    cfgToWg = n: v: let
  ...
  config.services.openvpn.servers = let
    cfgToOpenVPN = n: v: let
      interfaceName = "${v.peering.network}-${n}";
    in
      lib.nameValuePair interfaceName {
        config = ''
