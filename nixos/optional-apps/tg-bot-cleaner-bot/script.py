...
with TelegramClient("tg", api_id, api_hash) as client:
    ...
    async def handler(event):
        await client.send_read_acknowledge(
            event.message.peer_id, event.message, clear_mentions=True
        )
        ...
        if reply and len(reply.message) >= 1 and reply.message[0] in "/\\":
            print(
                "Bot %d new msg %d channel %d reply to %d"
            ...
            if l:
                print(
                    "Schedule del bot %d channel %d msg %s"
                ...
                if not no_delete:
                    ...
                    print(
                        "Actual del bot %d channel %d msg %s"
                        % (user_id, channel_id, ",".join([str(i) for i in l]))
                    )
        else:
            ...
            with db_lock:
                ...
                if reply.id in l:
                    ...
                    print(
                        "User %d reply %d to bot %d channel %d"
