...
let
  ...
  idToCoordinate = builtins.listToAttrs (
    builtins.map (v: {
...
in
{
  ...
  lantian.bgp-graph.eve = {
    ...
    nodes = builtins.map (planet: {
      name = "${planet.region}-${planet.name}";
      id = planet.id - ID_OFFSET;
    }) data.solarSystems;

    edges = builtins.map (jump: rec {
      ...
      distance =
        ...
        builtins.ceil (
          DISTANCE_RATIO
          * LT.math.sqrt ((LT.math.pow (x - x') 2) + (LT.math.pow (y - y') 2) + (LT.math.pow (z - z') 2))
        );
    }) data.jumps;
  };
}
