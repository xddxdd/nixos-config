...
init() {
    ...
    WAIT_GPIO
    echo "$SX1261_RESET_PIN" >/sys/class/gpio/export
    WAIT_GPIO
    echo "$SX1302_POWER_EN_PIN" >/sys/class/gpio/export
    WAIT_GPIO
    echo "$AD5338R_RESET_PIN" >/sys/class/gpio/export
    WAIT_GPIO
...
term() {
    ...
    if [ -d /sys/class/gpio/gpio$SX1261_RESET_PIN ]; then
        echo "$SX1261_RESET_PIN" >/sys/class/gpio/unexport
        WAIT_GPIO
    fi
